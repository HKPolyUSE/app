...
export default function VerticalScaleSimulation() {
  ...
  const nextUpgradeCost = TIERS[nextTierIndex]?.upgradeCost;
  ...
  const isDown = downtimeLeft > 0 || crash;
  ...
  const nextRound = () => {
    ...
    if (isDown) {
      ...
      churnMsg = `${churnedUsers} users lost while site was offline.`;
      churnType = "warnslow";
  ...
  const reset = () => {
    ...
    setTotalDowntime(0); setTotalCrashes(0); setBudget(1000); setLastProfit(0);
  ...
  const systemStatusLabel = isRecovering ? "Rebooting" : crash ? "Down" : downtimeLeft ? "Scaling" : utilization > 0.85 ? "High Load" : "Stable";
  ...
  return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 p-2 sm:p-4 font-sans text-slate-900 overflow-hidden">
      ...
      {showInstructions && (
        <Modal title="System Handbook" onClose={() => setShowInstructions(false)}>
          <div className="text-left text-xs space-y-2 leading-relaxed">
            ...
            <p>• <span className="font-bold">Scaling Costs:</span> Overhead increases per tier. Maintain profitability to survive.</p>
          </div>
        </Modal>
      )}
      ...
      <div className="flex flex-col w-full max-w-6xl h-full max-h-[900px] bg-white border border-slate-200 rounded-[2.5rem] shadow-xl overflow-hidden">
        ...
        <header className="flex-shrink-0 p-4 sm:px-8 border-b bg-white flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            {isCrashedWaiting ? (
              ...
              <button onClick={handleScaleUp} className="h-9 px-5 bg-pink-600 text-white rounded-lg font-bold hover:bg-pink-700 disabled:opacity-20 transition-all shadow-md" disabled={!canUpgradeStatus}>Scale Up</button>
            )}
        ...
        {/* CENTER CONTENT: Flexible height 60/40 Split */}
        <main className="flex-1 flex flex-col md:flex-row overflow-hidden border-b border-slate-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-6">
              <div className="space-y-3">
                ...
                {[
                  ...
                  { label: "Instance", value: currentTier.name },
                  { label: "Stability", value: <span className={`font-black ${stabilityColor}`}>{stability.toFixed(0)}%</span> },
              ...
              <div className="space-y-3">
                ...
                {[
                  ...
                  { label: "Latency", value: <span className={responseTime > 700 ? "text-rose-500 font-bold" : "text-emerald-600"}>{responseTime}ms</span> },
                  { label: "Utilization", value: crash ? "—" : <span className={utilization >= 0.85 ? "text-rose-500 font-black" : "text-emerald-500 font-bold"}>{(utilization * 100).toFixed(0)}%</span> },
                  ...
                  { label: "Overhead", value: <span className="text-rose-400 font-bold">-${maintenance}</span> }
                ].map((m, i) => (
                  <div key={i} className="flex justify-between items-center"><span className="text-slate-400 text-[10px] font-bold uppercase">{m.label}</span><div className="text-xs font-bold text-slate-800">{m.value}</div></div>
                ))}
              </div>
          ...
          <section className="w-full md:w-2/5 bg-slate-50/50 p-6 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className={`relative w-full max-w-[240px] p-1 rounded-[2rem] border-4 transition-all duration-700 shadow-xl ${crash ? 'border-rose-200 bg-rose-50' : isDown ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}>
              <div className="p-5 space-y-3">
                ...
                <div className={`p-4 rounded-2xl border transition-all flex flex-col items-center gap-1 ${isDown ? 'bg-slate-100 text-slate-400 grayscale' : 'bg-indigo-50 border-indigo-100 text-indigo-600 shadow-sm'}`}>
                  ...
                  <div className="w-full bg-indigo-200 h-1 rounded-full overflow-hidden">
                    <div className="bg-indigo-500 h-full transition-all duration-700" style={{ width: crash ? '0%' : `${utilization * 100}%` }}></div>
                  </div>
                </div>
                {/* DB Tier */}
                <div className={`p-4 rounded-2xl border transition-all flex flex-col items-center gap-1 ${isDown ? 'bg-slate-100 text-slate-400 grayscale' : 'bg-emerald-50 border-emerald-100 text-emerald-600 shadow-sm'}`}>
                  ...
                  <div className="w-full bg-emerald-200 h-1 rounded-full overflow-hidden">
                    <div className="bg-emerald-500 h-full transition-all duration-700" style={{ width: crash ? '0%' : `${Math.min(utilization * 1.1, 1) * 100}%` }}></div>
                  </div>
                </div>
                {/* Specs */}
            ...
            <p className="mt-4 text-[9px] text-slate-400 font-bold text-center leading-tight">MONOLITHIC TOPOLOGY</p>
          </section>
        </main>
